/*
Copyright 2010 MCForge team (Modified for use with SinCraft)
...
namespace SinCraft
{
    public sealed partial class Player : IDisposable
    {
        ...
        /// <summary>
        /// The player's set custom name.
        /// </summary>
        public string SetName;
        [Obsolete("Use Player.Username instead for the player's actual username, or Player.SetName for the player's set custom name.")]
        ...
        public Player(string playername) { Username = playername; }
        ...
        public Player(Socket s)
        {
            try
            {
                ...
                extraTimer.Elapsed += delegate
                {
                    ...
                    try
                    {
                        if (!Group.Find("Nobody").commands.Contains("pay") && !Group.Find("Nobody").commands.Contains("give") && !Group.Find("Nobody").commands.Contains("take")) SendMessage("You currently have &a" + Money + Server.DefaultColor + " " + Server.Currency);
                    }
                    catch { }
                    SendMessage("You have modified &a" + BlocksModified + Server.DefaultColor + " blocks!");
                    if (players.Count == 1)
        ...
        public void save()
        {
            ...
            string commandString =
                ...
                ", totalDeaths=" + Deaths +
                ", Money=" + Money +
                ", totalBlocks=" + BlocksModified + " + " + loginBlocks +
                ", totalKicked=" + TimesKicked +
                ", TimeSpent='" + TimeSpent +
                "' WHERE Name='" + Username + "'";
            if (MySQLSave != null)
        ...
        void HandleLogin(byte[] message)
        {
            try
            {
                ...
                if (Username.Split('@').Length > 1)
                {
                    ...
                    Username += "_" + MojangAccount.GetID(truename);
                }
                string verify = enc.GetString(message, 65, 32).Trim();
                byte type = message[129];

                isDev = Server.Devs.Contains(Username.ToLower());
                isProtected = isDev;
                ...
                if (Server.useWhitelist)
                {
                    if (Server.verify)
                    {
                        if (Server.whiteList.Contains(Username))
                        {
                            onWhitelist = true;
                        }
                    }
                    else
                    {
                        ...
                        {
                            if (ipQuery.Rows.Contains(Username) && Server.whiteList.Contains(Username))
                            {
                                onWhitelist = true;
                            }
                        }
                    ...
                    if (isDev) onWhitelist = true;
                    if (!onWhitelist) { Kick("This is a private server!"); return; }
                }

                //premium check
                if (Server.PremiumPlayersOnly && !isDev)
                {
                    using (WebClient Client = new WebClient())
                    {
                        ...
                        while (tries++ < 3)
                        {
                            try
                            {
                                bool haspaid = Convert.ToBoolean(Client.DownloadString("http://www.minecraft.net/haspaid.jsp?user=" + Username));
                                if (!haspaid)
                ...
                string temp = Username + " is lately known as:";
                bool found = false;
            ...
            Database.AddParams("@Name", Username);
            DataTable playerDb = Database.fillData("SELECT * FROM Players WHERE Name=@Name");

            DataTable setNameExists = SQLite.fillData("SELECT Name FROM Players WHERE Name='SetName'");
            if (setNameExists.Columns.Count == 0)
            {
                SQLite.executeQuery("ALTER TABLE Players ADD COLUMN SetName TEXT");
                //else SQLite.executeQuery("ALTER TABLE Players ADD COLUMN SetName VARCHAR(6) AFTER totalKicked");
            }
            ...
            if (playerDb.Rows.Count == 0)
            {
                ...
                SendMessage("Welcome " + Username + "! This is your first visit.");
                Database.executeQuery(String.Format("INSERT INTO Players (Name, SetName, IP, FirstLogin, LastLogin, totalLogin, Title, totalDeaths, Money, totalBlocks, totalKicked, TimeSpent) VALUES ('{0}', '{1}', '{2}', '{3:yyyy-MM-dd HH:mm:ss}', '{4:yyyy-MM-dd HH:mm:ss}', {5}, '{6}', {7}, {8}, {9}, {10}, '{11}')", Username, SetName, ip, firstLogin, DateTime.Now, Logins, prefix, Deaths, Money, loginBlocks, TimesKicked, TimeSpent));
                Database.executeQuery("INSERT INTO Economy (player, money, total, purchase, payment, salary, fine) VALUES ('" + Username + "', " + Money + ", 0, '%cNone', '%cNone', '%cNone', '%cNone')");
            }
            else
            {
                SetName = playerDb.Rows[0]["SetName"].ToString().Trim();
                Logins = int.Parse(playerDb.Rows[0]["totalLogin"].ToString()) + 1;
                ...
                Deaths = int.Parse(playerDb.Rows[0]["TotalDeaths"].ToString());
                BlocksModified = long.Parse(playerDb.Rows[0]["totalBlocks"].ToString().Trim());
                //money = int.Parse(playerDb.Rows[0]["Money"].ToString());
                Money = Economy.RetrieveEcoStats(this.Username).money;
                TimesKicked = int.Parse(playerDb.Rows[0]["totalKicked"].ToString());
                SendMessage("Welcome back " + color + prefix + Username + Server.DefaultColor + "! You've been here " + Logins + " times!");
                if (Server.muted.Contains(Username))
                {
                    muted = true;
            ...
            try
            {
                ...
                foreach (Player p in players)
                {
                    if (p.level == level && p != this && !p.hidden)
                        SendSpawn(p.id, p.color + p.Username, p.pos[0], p.pos[1], p.pos[2], p.rot[0], p.rot[1]);
                }
            ...
            {
                Server.ErrorLog(e);
                Server.s.Log("Error spawning player \"" + Username + "\"");
            }
            ...
            string joinm = "&a+ " + this.color + this.prefix + this.SetName + Server.DefaultColor + " " + File.ReadAllText("text/login/" + this.name + ".txt");
            if (this.group.Permission < Server.adminchatperm || Server.adminsjoinsilent == false)
            {
            ...
            Server.s.Log(Username + " [" + ip + "] has joined the server.");
        ...
        {
            string viptitle = isDev ? string.Format("{1}[{0}Dev{1}] ", c.Parse("blue"), color) : "";
        ...
        void HandleBlockchange(byte[] message)
        {
            ...
            try
            {
                ...
                if (action == 1 && Server.ZombieModeOn && Server.noPillaring)
                {
                    if (!referee)
                    {
                        ...
                        if (blocksStacked == 4)
                        {
                            Command.all.Find("kick").Use(null, Username + " No pillaring allowed!");
                        }
                    }
                }
            ...
            catch (Exception e)
            {
                ...
                GlobalMessageOps(Username + " has triggered a block change error");
        ...
        public void manualChange(ushort x, ushort y, ushort z, byte action, byte type)
        {
            ...
            if (b == Block.griefer_stone && group.Permission <= Server.grieferStoneRank && !isDev)
            {
            ...
            if (lastDeath.AddSeconds(2) < DateTime.Now)
            {
                ...
                if (level.Killer && !invincible)
                {
                    ...
                    if (Server.deathcount)
                        if (Deaths > 0 && Deaths % 10 == 0) GlobalChat(this, this.color + this.prefix + this.name + Server.DefaultColor + " has died &3" + Deaths + " times", false);
                }
        ...
        void HandleChat(byte[] message)
        {
            try
            {
                ...
                if (text.Truncate(6) == "/womid")
                {
                    ...
                    Player.GlobalMessage(c.red + "[INFO] " + color + SetName + "%f is using wom client");
                    Player.GlobalMessage(c.red + "[INFO] %fVersion: " + version);
                    Server.s.Log(c.red + "[INFO] " + color + Username + "%f is using wom client");
                ...
                if (text != "/afk")
                {
                    if (Server.afkset.Contains(this.Username))
                    {
                        Server.afkset.Remove(this.Username);
                        Player.GlobalMessage("-" + this.color + this.SetName + Server.DefaultColor + "- is no longer AFK");
            ...
            hello:
                ...
                if (muted && !isDev) { this.SendMessage("You are muted."); return; } //Muted: Only allow commands
                ...
                if (Server.checkspam == true)
                {
                    ...
                    // consecutivemessages++;
                    //}
                    if (Player.lastMSG == this.Username)
                    {
                        consecutivemessages++;
                    }
                    else
                    ...
                    if (this.consecutivemessages >= Server.spamcounter)
                    {
                        ...
                        Command.all.Find("mute").Use(null, this.Username);
                        Player.GlobalMessage(this.SetName + " has been &0muted &efor spamming!");
                ...
                if (text.Length >= 2 && text[0] == '@' && text[1] == '@')
                {
                    ...
                    Server.s.Log("[>] " + this.SetName + ": " + text);
                    return;
                }
                if (text[0] == '@' || whisper)
                {
                ...
                if (text[0] == '#' || opchat)
                {
                    ...
                    if (group.Permission < Server.opchatperm && !isDev)
                ...
                if (text[0] == '+' || adminchat)
                {
                    ...
                    if (group.Permission < Server.adminchatperm && !isDev)
                ...
                if (text[0] == ':')
                {
                    if (PlayingTntWars)
                    {
                        ...
                        if (it.GameMode == TntWarsGame.TntWarsGameMode.TDM)
                        {
                            ...
                            {
                                if (pl.Red && p.Red) SendMessage(p.p, "To Team " + c.red + "-" + color + SetName + c.red + "- " + Server.DefaultColor + newtext);
                                if (pl.Blue && p.Blue) SendMessage(p.p, "To Team " + c.blue + "-" + color + SetName + c.blue + "- " + Server.DefaultColor + newtext);
                            }
                            Server.s.Log("[TNT Wars] [TeamChat (" + (pl.Red ? "Red" : "Blue") + ") " + SetName + " " + newtext);
                            return;
                        }
                    }
                }
...
return;
                ...
                if (!level.worldChat)
                {
                    Server.s.Log("<" + SetName + ">[level] " + text);
        ...
        public void HandleCommand(string cmd, string message)
        {
            try
            {
                ...
                if (Server.MustReadRules && !agreed)
                {
                ...
                if (cmd.ToLower() == "pony")
                {
                    if (ponycount < 2)
                    {
                        GlobalMessage(this.color + this.SetName + Server.DefaultColor + " just so happens to be a proud brony! Everyone give " + this.color + this.name + Server.DefaultColor + " a brohoof!");
                        ponycount += 1;
                    }
                    else
                    {
                ...
                if (CommandHasBadColourCodes(this, message))
                    return;
                /*string foundShortcut = Command.all.FindCmdNameByAlias(cmd);
                ...
                if (command != null)
                {
                    ...
                    {
                        SendMessage("Cannot use command, player is protected.");
                        Server.s.CommandUsed(Username + " used /" + cmd + " " + message);
                        return;
                    }
                    if (group.CanExecute(command) || isDev)
                    {
                        ...
                        if ((this.joker == true || this.muted == true) && !isDev)
                        {
                            if (cmd.ToLower() == "me")
                            {
        ...
        public void SendMessage(byte id, string message, bool colorParse)
        {
            ...
            sb.Replace("$deaths", Deaths.ToString());
            sb.Replace("$money", Money.ToString());
            sb.Replace("$blocks", BlocksModified.ToString());
            sb.Replace("$first", firstLogin.ToString());
            sb.Replace("$kicked", TimesKicked.ToString());
...
changed |= 4;*/
        ...
        public static void GlobalChat(Player from, string message, bool showname)
        {
            ...
            if (showname)
            {
                ...
                message = referee + from.color + from.voicestring + from.color + from.prefix + from.SetName + ": &f" + message;
            }
            ...
            foreach (string s in checkmessagesplit)
            {
                ...
                if (s.StartsWith("%"))
                {
                    if (lastendwithcolour)
                    {
                        if (who != null)
                        {
                            who.SendMessage("Sorry, your color codes in this command were invalid (You cannot use 2 color codes next to each other");
                            who.SendMessage("Command failed.");
                            Server.s.Log(who.name + " attempted to send a command with invalid colours codes (2 color codes were next to each other)!");
                            GlobalMessageOps(who.color + who.SetName + " " + Server.DefaultColor + " attempted to send a command with invalid color codes (2 color codes were next to each other)!");
                        }
                        return true;
                    }
                    else if (s.Length == 2)
        ...
        public static void GlobalMessageOps(string message)
        {
            try
            {
                players.ForEach(delegate(Player p)
                {
                    if (p.group.Permission >= Server.opchatperm || p.isDev)
                    { //START
                        Player.SendMessage(p, message);
                    }
                });
        ...
        public static void GlobalMessageAdmins(string message)
        {
            try
            {
                players.ForEach(delegate(Player p)
                {
                    if (p.group.Permission >= Server.adminchatperm || p.isDev)
                    {
                        Player.SendMessage(p, message);
                    }
                });
            ...
            try
            {
                ...
                if (loggedIn)
                {
                    ...
                    if (kickString == "Disconnected." || kickString.IndexOf("Server shutdown") != -1 || kickString == Server.customShutdownMessage)
                    {
                        ...
                        if (!hidden)
                        {
                            string leavem = "&c- " + color + prefix + SetName + Server.DefaultColor + " " + File.ReadAllText("text/logout/" + name + ".txt");
                    ...
                    {
                        TimesKicked++;
                        GlobalChat(this, "&c- " + color + prefix + SetName + Server.DefaultColor + " kicked (" + kickString + Server.DefaultColor + ").", false);
...
//this.commThread.Abort();
        ...
        public static Player Find(string name)
        {
            ...
            foreach (Player pl in tempList)
            {
                if (pl.Username.ToLower() == name.ToLower()) return pl;
                if (pl.Username.ToLower().IndexOf(name.ToLower()) != -1)
                {
                    if (tempPlayer == null) tempPlayer = pl;
